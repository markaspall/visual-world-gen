...
export class NormalizeNode extends BaseNode {
  ...
  async process(inputs, params) {
    ...
    // Create buffers
    const inputBuffer = this.createDataBuffer(data.byteLength);
    this.device.queue.writeBuffer(inputBuffer, 0, data);
    
    const outputBuffer = this.createDataBuffer(data.byteLength);
    ...
    const output = await this.downloadData(outputBuffer, data.byteLength);
