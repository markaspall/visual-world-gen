...
/**
 * Post-Erosion Moisture Node - LOD 0 (128×128)
 * Different moisture sample - creates dry eroded areas and wet uneroded areas
 */
export class PostErosionMoistureNode extends BaseNode {
  static inputs = ['seed'];
  static outputs = ['moisture'];
  static defaultParams = {
    resolution: 128,
    frequency: 0.0012,  // Different from pre-erosion (0.001)
    octaves: 3,
    persistence: 0.5
  };
  ...
  async process(inputs, params) {
    ...
    const shaderCode = `
      ...
      fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
        ...
        // Use DIFFERENT seed offset (5000) to get different pattern
        let seed_x = f32((params.seed + 5000u) % 10000u) * 0.001;
        let seed_y = f32(((params.seed + 5000u) / 10000u) % 10000u) * 0.001;
    ...
    console.log(`✅ PostErosionMoisture generated (${resolution}×${resolution})`);
    return { moisture };
  }
}
